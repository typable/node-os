...
public class JsonParser implements Parser<JsonObject, String> {
	...
	@Override
	public void parse(JsonObject target, String source) {
		...
		for(String s : splits) {
			...
			if(start == '{' && end == '}') {
				...
				for(int i = 0; i < splits_.length; i++) {
					...
					obj__[i] = new JsonObject();
					
					parse((JsonObject) obj__[i], splits_[i]);
				}
	...
	@Override
	public void compose(String target, JsonObject source) {
		...
		for(String key : source.keys()) {
			...
			if(obj_ instanceof JsonObject) {
				...
				compose(target, (JsonObject) obj_);
			}
